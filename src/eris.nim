...
type
  ...
  ErisStoreObj* = object of RootObj
    getImpl*: proc (s: ErisStore; r: Reference): Future[seq[byte]] {.nimcall,
        gcsafe.}
    putImpl*: proc (s: ErisStore; r: Reference; f: PutFuture) {.nimcall, gcsafe.}

using store: ErisStore
proc discardPut(store; r: Reference; f: PutFuture) =
  f.complete()
...
proc put*(store; r: Reference; f: PutFuture) =
  ## Put the block in ``f`` for ``Reference`` ``r`` into ``store``.
  assert(not store.putImpl.isNil)
  assert(f.mget.len in blockSizes)
  store.putImpl(store, r, f)
